# %% [markdown]
# # 🚗 TrafficGamer - Pure Waymax Training on Kaggle (v3 - Batch Training Fixed)
#
...
for batch_idx in tqdm(range(num_batches), desc="Training Batches"):
    ...
    for ep_in_batch in range(BATCH_SIZE):
        ...
        for step in range(CONFIG['steps_per_episode']):
            ...
            # Get actions from agents (and store action distributions for magnet)
            actions = []
            action_dists = []
            for i, agent in enumerate(agents):
                ...
                actions.append(action.squeeze(0))  # Remove batch dim
    ...
    # Log progress
    if (batch_idx + 1) % (CONFIG['log_interval'] // BATCH_SIZE + 1) == 0:
...
# - ✅ v3: **Fixed magnet field - store action distributions**
#
# **Repository:** https://github.com/PhamPhuHoa-23/EvolutionaryTest
