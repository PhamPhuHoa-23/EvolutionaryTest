# %% [markdown]
# # 🚗 TrafficGamer - Pure Waymax Training on Kaggle (v4 - All Fixes)
#
...
# - ✅ **FIXED v4: Disabled magnet to avoid log_prob tensor issue**
#
...
CONFIG = {
    ...
    # Magnet (exploration) - DISABLED due to log_prob tensor shape issue in original code
    # The algorithm expects magnet[i].log_prob(actions[i]).item() but log_prob returns multi-dim tensor
    'is_magnet': False,  # Set to False to avoid "only one element tensors" error
    'eta_coef1': 0.05,
...
rl_config = {
    ...
    # Magnet settings - DISABLED to avoid tensor shape issue
    'is_magnet': CONFIG['is_magnet'],  # False
...
if not CONFIG['is_magnet']:
    print("\n⚠️  Note: Magnet exploration is DISABLED to avoid tensor shape error in algorithm code")
...
#
# **FIXED v3:** Collect `batch_size` episodes before calling `agent.update()`
# **FIXED v4:** Disabled magnet to avoid tensor shape error
    ...
    for ep_in_batch in range(BATCH_SIZE):
        ...
        if len(controlled_agents) < NUM_AGENTS:
            ...
            for i in range(NUM_AGENTS):
                ...
                # Magnet: use a dummy distribution (won't be used since is_magnet=False)
        ...
        for step in range(CONFIG['steps_per_episode']):
            ...
            for i, agent in enumerate(agents):
                ...
                # Get distribution for magnet (even if disabled, need to fill the field)
            ...
            for i, agent_idx in enumerate(controlled_agents):
                ...
                # Store action distribution for magnet (won't be used since is_magnet=False)
...
# - ✅ v4: **Disabled magnet** (`is_magnet=False`) to avoid "only one element tensors" error
#
# **Note:** The magnet feature has a bug in the original TrafficGamer code where 
# `log_prob()` returns a multi-dimensional tensor but the code expects a scalar.
# To fix this properly, the algorithm code needs to be updated to use `.sum()` on log_prob.
#
# **Repository:** https://github.com/PhamPhuHoa-23/EvolutionaryTest
